@section('title', 'Pusat Arsip Digital')
...
<x-app-layout>
    ...
    <div class="mb-6 md:mb-8 flex flex-col sm:flex-row sm:items-center justify-between gap-4 relative z-20">
        <div>
            <h2 class="text-2xl md:text-3xl font-black text-zinc-900 dark:text-white tracking-tight flex items-center gap-2">
                <i class="bi bi-archive-fill text-amber-500"></i> Arsip Rapor Digital
            </h2>
            <p class="text-xs md:text-[13px] font-medium text-zinc-500 dark:text-zinc-400 mt-0.5">
                Kelola dan telusuri dokumen (E-Document) yang telah disahkan dan dibekukan secara permanen.
            </p>
        </div>
    </div>
    ...
    <div class="m3-glass-card p-4 sm:p-5 mb-5 relative z-10 shadow-2xs">
        <form action="{{ route('arsip-rapor.index') }}" method="GET" class="flex flex-col lg:flex-row gap-2.5">
            ...
            <div class="w-full lg:w-40 relative group">
                <div
                    class="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none text-zinc-400 group-focus-within:text-primary transition-colors">
            ...
            <div class="w-full lg:w-40 relative group">
                <div
                    class="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none text-zinc-400 group-focus-within:text-primary transition-colors">
                    <i class="bi bi-journal-bookmark-fill text-xs"></i>
                </div>
                <select name="ujian" onchange="this.form.submit()"
                    class="m3-input-glass w-full !pl-9 !pr-8 text-xs font-bold cursor-pointer appearance-none">
                    <option value="" class="text-zinc-500">Semua Ujian</option>
                    @foreach ($daftarUjian as $ujn)
                        <option value="{{ $ujn->nama_ujian }}"
                            {{ request('ujian') == $ujn->nama_ujian ? 'selected' : '' }}>
                            {{ $ujn->nama_ujian }}
                        </option>
                    @endforeach
                </select>
            ...
            <!-- Tombol Cari -->
            <button type="submit"
                class="hidden lg:flex h-10 px-5 bg-primary hover:bg-primary-dark text-white rounded-xl text-xs font-black shadow-2xs transition-all active:scale-95 items-center justify-center outline-none">
                Saring
            </button>
        </form>
    </div>
            ...
            <table class="w-full text-left border-collapse text-xs min-w-[850px]">
                <thead class="bg-zinc-100/70 dark:bg-zinc-800/50 border-b border-zinc-200/80 dark:border-zinc-800">
                    <tr class="text-[10px] font-black uppercase tracking-wider text-zinc-500 dark:text-zinc-400">
                        ...
                        <th class="py-3 px-4 border-r border-zinc-200/60 dark:border-zinc-800/60 w-44">Tanggal & Jenis</th>
                        <th class="py-3 px-5 border-r border-zinc-200/60 dark:border-zinc-800/60">Informasi Dokumen</th>
                    ...
                    @forelse ($arsips as $index => $arsip)
                        ...
                        <tr class="hover:bg-zinc-50/50 dark:hover:bg-zinc-800/30 transition-colors">
                            ...
                            <td class="py-3 px-4 border-r border-zinc-200/60 dark:border-zinc-800/60 align-top">
                                ...
                                <span
                                    class="inline-flex items-center px-2 py-0.5 rounded-lg text-[9px] font-black uppercase tracking-wider bg-blue-500/10 text-blue-600 dark:text-blue-400 border border-blue-500/20 shadow-2xs">
                                    {{ str_replace('_', ' ', $arsip->tipe_dokumen) }}
                                </span>
                            </td>
                            ...
                            <td class="py-3 px-5 border-r border-zinc-200/60 dark:border-zinc-800/60 align-top">
                                <p class="font-black text-sm text-zinc-900 dark:text-white mb-0.5 uppercase tracking-tight">
                                    {{ $data['nama_murid'] ?? 'Tidak Diketahui' }}
                                </p>
                                <p class="text-[11px] font-bold text-zinc-500 dark:text-zinc-400">
                                    {{ $data['nama_ujian'] ?? '-' }} &bull; {{ $data['nama_ruangan'] ?? '-' }}
                                </p>
                                <p
                                ...
                                <div class="flex flex-col gap-1.5">
                                    @if ($arsip->tipe_dokumen === 'rapor_murid')
                                        <a href="{{ route('arsip.cetak', $arsip->id) }}" target="_blank"
                                            ...
                                            <i class="bi bi-printer-fill"></i> Lihat
                                        </a>
                                    @else
                                        <span class="text-[10px] font-bold text-zinc-400 italic">Belum Tersedia</span>
                                    @endif
                                </div>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="5" class="py-12 text-center">
                                <x-empty-state icon="bi-archive" title="Arsip Kosong" message="Belum ada dokumen yang disahkan dan dibekukan. Atau data yang Anda cari tidak ditemukan." />
                            </td>
                        </tr>
                    @endforelse
                </tbody>
