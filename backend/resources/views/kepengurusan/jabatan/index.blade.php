@section('title', 'Jabatan Pengurus')
...
<x-app-layout>
    ...
    <div class="mb-6 md:mb-8 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 relative z-10">
        <div>
            ...
            <p class="text-xs md:text-[13px] font-medium text-zinc-500 dark:text-zinc-400 mt-0.5">
                Kelola data master hierarki dan jabatan kepengurusan yayasan.
            </p>
        </div>
        ...
        <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-2.5 w-full sm:w-auto">
            ...
            <!-- Search Form -->
            <form action="{{ route('jabatan-pengurus.index') }}" method="GET"
                class="relative w-full sm:w-72 group/search">
                <div
                ...
                @if (request('search'))
                    <a href="{{ route('jabatan-pengurus.index') }}"
            ...
            <!-- Add Button -->
            @can('create jabatan-pengurus')
                <a href="{{ route('jabatan-pengurus.create') }}"
                    class="m3-btn-primary w-full sm:w-auto action-modal group/btn h-10 px-5 text-xs font-black shadow-2xs">
    ...
    <div id="data-grid-container" class="flex flex-col gap-3 relative z-10">
        @include('kepengurusan.jabatan.list', ['jabatan' => $jabatan])
    </div>
</x-app-layout>
