...
<x-app-layout>
    ...
    <div class="mb-6 md:mb-8 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 relative z-10">
        <div>
            ...
            <p class="text-[13px] font-semibold text-zinc-500 dark:text-zinc-400 mt-0.5 transition-colors duration-300">
                Kelola data Level dan tingkatan kelas pendidikan.
            </p>
        </div>
        ...
        <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-2.5 w-full sm:w-auto">
            ...
            <!-- Search Form -->
            <form action="{{ route('level.index') }}" method="GET" class="relative w-full sm:w-72 group/search">
                <div
                ...
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari level..."
            ...
            <!-- Add Button -->
            @can('create level')
                <a href="{{ route('level.create') }}" class="m3-btn-primary w-full sm:w-auto action-modal group/btn">
    ...
    <div id="data-grid-container" class="flex flex-col gap-3 relative z-10">
        @include('level.list', ['groupedLevels' => $groupedLevels])
    </div>
</x-app-layout>
