...
<x-app-layout>
    ...
    <!-- Header Page & Actions -->
    <div
        class="mb-6 md:mb-8 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 relative z-20 print:hidden">
        <div>
            ...
            <p class="text-xs font-bold text-zinc-500 dark:text-zinc-400 mt-0.5 uppercase tracking-wider">
                Generate 7 digit nomor seri barcode unik universal (500 baris) untuk dikirim ke percetakan buku tabungan
            </p>
        </div>
    ...
    <div class="m3-glass-card p-5 mb-6 rounded-3xl relative z-10 shadow-2xs print:hidden">
        <form action="{{ route('tabungan.barcode.generator') }}" method="GET"
            ...
            <!-- 1. Prefix Seri Barcode -->
            <div>
                <label
                    class="block text-xs font-black uppercase tracking-wider text-zinc-700 dark:text-zinc-300 mb-1.5">
                    Prefix Nomor Seri (Universal):
                </label>
                <input type="text" name="prefix" value="{{ $prefix }}" maxlength="4" required
                    placeholder="Contoh: 1000" class="m3-input-glass w-full text-xs font-mono font-bold text-primary">
            </div>
            ...
            <div>
                ...
                <input type="number" name="jumlah" value="{{ $jumlah }}" min="10" max="1000"
                    step="10" class="m3-input-glass w-full text-xs font-mono font-bold">
            </div>
    ...
    <div
        class="p-5 rounded-3xl bg-primary/10 border border-primary/20 flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 shadow-2xs">
        <div class="flex items-center gap-3.5">
            <div
                class="w-12 h-12 rounded-2xl bg-primary/20 text-primary dark:text-primary-dark flex items-center justify-center text-xl font-bold border border-primary/30 shrink-0">
                <i class="bi bi-upc-scan"></i>
            </div>
            <div>
                <h3 class="text-sm font-black text-zinc-900 dark:text-white">
                    Format 7 Digit Universal: <span
                        class="font-mono text-primary dark:text-primary-dark text-base">{{ $prefix }}001</span>
                    s.d. <span
                        class="font-mono text-primary dark:text-primary-dark text-base">{{ $prefix }}{{ str_pad($mulaiDari + $jumlah - 1, 3, '0', STR_PAD_LEFT) }}</span>
                </h3>
                <p class="text-xs text-zinc-600 dark:text-zinc-400 mt-0.5">
                    Prefix Seri: <strong class="font-mono font-black text-primary">{{ $prefix }}</strong> &bull;
                    Total Dihasilkan: <strong class="text-zinc-900 dark:text-white">{{ count($barcodes) }} Baris
                        Barcode</strong>
                </p>
            </div>
        </div>
        <div class="flex items-center gap-2 self-start sm:self-center">
            <span
                class="px-3 py-1 rounded-xl bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 text-xs font-black border border-emerald-500/20">
                Format Seri Universal (Siap Cetak)
            </span>
        </div>
    </div>
    ...
    <div class="m3-glass-card overflow-hidden rounded-3xl shadow-2xs relative z-10">
        <div
            class="p-4 sm:p-5 border-b border-zinc-200/80 dark:border-zinc-800 bg-zinc-50/60 dark:bg-zinc-950/40 flex justify-between items-center">
            <span
                    ...
                    <div onclick="copySingleBarcode('{{ $item['barcode'] }}', this)"
                        ...
                        <div class="flex items-center justify-between">
                            ...
                            @if ($item['is_registered'])
                                <span
                                    class="px-1.5 py-0.5 rounded text-[8px] font-black uppercase bg-emerald-500/10 text-emerald-600 border border-emerald-500/20">
                                    Dipakai
                                </span>
                            @else
                                <span
                                    class="px-1.5 py-0.5 rounded text-[8px] font-black uppercase bg-zinc-100 text-zinc-500 dark:bg-zinc-800 dark:text-zinc-400">
                                    Tersedia
                                </span>
                            @endif
                        </div>
                        <div class="text-center py-1">
                            <span
                                class="font-mono font-black text-sm tracking-wider text-zinc-900 dark:text-white group-hover/card:text-primary transition-colors block">
                                {{ $item['barcode'] }}
                            </span>
                        ...
                        <div
                            class="text-[9px] text-center text-primary dark:text-primary-dark font-bold opacity-0 group-hover/card:opacity-100 transition-opacity">
    ...
    <textarea id="rawBarcodesText" class="sr-only">
