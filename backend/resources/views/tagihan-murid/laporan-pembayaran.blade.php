...
<x-app-layout>
    ...
    <div class="grid grid-cols-1 xl:grid-cols-3 gap-4 md:gap-5 mb-6 md:mb-8 relative z-10 print:hidden">
        ...
        <div class="xl:col-span-2 m3-glass-card p-4 md:p-5 shadow-2xs flex flex-col justify-center">
            <form action="{{ route('pembayaran-tagihan.laporan') }}" method="GET"
                class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-12 gap-3 items-end w-full">
                ...
                <div class="md:col-span-4 sm:col-span-2">
                    ...
                    <div class="relative h-10">
                        ...
                        <select name="jenis_biaya"
                            ...
                            <option value="SPP" selected>SPP / Syahriyah Rutin</option>
                        </select>
                        <div
                            class="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none text-zinc-400">
                            <i class="bi bi-lock-fill text-[10px] font-black"></i>
                        </div>
                    </div>
                </div>

                <!-- Tombol Action -->
                <div class="md:col-span-12 sm:col-span-2 mt-1">
                    <button type="submit"
        ...
        <div
            ...
            <div class="relative z-10">
                <p class="text-[10px] font-black text-sky-100 uppercase tracking-wider mb-1 flex items-center">
                    <i class="bi bi-wallet2 mr-1.5 text-xs"></i> Total Pendapatan SPP
                </p>
            ...
            <table class="w-full text-left border-collapse min-w-[1000px] text-xs">
                <thead
                    ...
                    <tr class="text-[10px] font-black uppercase tracking-wider text-zinc-500 dark:text-zinc-400">
                        ...
                        <th class="px-4 py-3 border-r border-zinc-200/80 dark:border-zinc-800 w-64">Murid (NISM)</th>
                        <th
                            class="px-4 py-3 border-r border-zinc-200/80 dark:border-zinc-800 w-72 text-emerald-600 dark:text-emerald-400">
                            Rincian SPP</th>
                        <th class="px-4 py-3 border-r border-zinc-200/80 dark:border-zinc-800 w-32 text-center">Sumber
                        </th>
                        ...
                        @php
                            ...
                            foreach ($rincianTagihan as $item) {
                                ...
                                if (str_contains($namaClean, 'spp') || str_contains($namaClean, 'syahriyah')) {
                                    $rincianSPP->push($item);
                                }
                            }

                            $murids = $rincianSPP->pluck('murid')->filter()->unique('id');
                        @endphp

                        @if ($rincianSPP->count() > 0)
                            <tr
                                            ...
                                            @foreach ($murids as $murid)
                                                <div>
                                                    ...
                                                    <div class="flex gap-1.5 items-center">
                                                        ...
                                                        @php
                                                            $ruangannya =
                                                                $rincianSPP->where('murid_id', $murid->id)->first()
                                                                    ->ruangan ?? null;
                                                        @endphp
                                                        <div
                                                            class="text-[8px] font-black text-zinc-400 tracking-wider uppercase truncate bg-zinc-100 dark:bg-zinc-800 px-1.5 py-0.5 rounded">
                                ...
                                <td class="px-4 py-3 align-top border-r border-zinc-200/60 dark:border-zinc-800">
                                    <div class="flex flex-wrap gap-1">
                                        @foreach ($rincianSPP as $spp)
                                            <span
                                                class="bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-500/20 px-1.5 py-0.5 rounded text-[9px] font-black uppercase tracking-wider truncate max-w-[200px]"
                                                title="{{ $spp->murid->nama_lengkap ?? '' }} - {{ $spp->nama_tagihan_spesifik }}">
                                                {{ str_replace(['SPP ', 'Syahriyah '], '', $spp->nama_tagihan_spesifik) }}
                                            </span>
                                        @endforeach
                                    </div>
                                </td>
                                ...
                                <td
                                    ...
                                    <span
                                        class="px-2 py-0.5 rounded-lg text-[9px] font-black uppercase tracking-wider border {{ $transaksi->tipe_pembayar == 'Donatur' ? 'bg-purple-500/10 text-purple-600 dark:text-purple-400 border-purple-500/20' : 'bg-sky-500/10 text-sky-600 dark:text-sky-400 border-sky-500/20' }}">
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="p-8 text-center">
                                <x-empty-state icon="bi-inboxes" title="Tidak Ada Transaksi SPP"
                                    message="Tidak ada transaksi SPP pada rentang tanggal tersebut." />
                            </td>
                        </tr>
                    @endforelse
                </tbody>
