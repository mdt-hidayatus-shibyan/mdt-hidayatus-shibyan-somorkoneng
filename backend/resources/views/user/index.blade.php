    ...
    <div class="mb-6 md:mb-8 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 relative z-10">
        <div>
            ...
            <p class="text-xs md:text-[13px] font-medium text-zinc-500 dark:text-zinc-400 mt-0.5">
                Monitoring status online, sesi login, dan akses akun seluruh pengguna sistem.
            </p>
        </div>
        ...
        <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-2.5 w-full sm:w-auto">
            ...
            <!-- Search Form -->
            <form action="{{ route('user.index') }}" method="GET" class="relative w-full sm:w-72 group/search">
                <div
            ...
            <!-- Add Button -->
            @can('tambah user')
                <a href="{{ route('user.create') }}" class="m3-btn-primary w-full sm:w-auto action-modal group/btn h-10 px-5 text-xs font-black shadow-2xs">
